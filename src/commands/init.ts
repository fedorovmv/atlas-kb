...
export async function initMemory(options: { root?: string; force?: boolean; dryRun?: boolean; installHooks?: boolean; installCi?: boolean } = {}) {
  ...
  // Generate wrapper script .ai/memory-tool/bin/memory
  // Работает в любом проекте (не требует package.json).
  // Предпочитает dist/cli.js (node), fallback на src/cli.ts через npx tsx.
  const wrapperDir = path.join(root, ".ai", "memory-tool", "bin");
  const wrapperPath = path.join(wrapperDir, "memory");
  ...
  const wrapperContent = [
    "#!/usr/bin/env bash",
    "# Auto-generated by repo-memory init. Wrapper for repo-memory CLI.",
  ...
  if (!options.dryRun && (!existsSync(wrapperPath) || options.force)) {
    ...
    written.push(".ai/memory-tool/bin/memory");
  }

  const packageJsonPath = path.join(root, "package.json");
  const packageHint = existsSync(packageJsonPath)
    ? "Optional: add script \"memory\": \".ai/memory-tool/bin/memory\" to package.json for convenience."
    : "No package.json needed. Use .ai/memory-tool/bin/memory directly.";
  ...
  console.log(`- Run: .ai/memory-tool/bin/memory validate`);
  console.log(`- Run: .ai/memory-tool/bin/memory context "изменить Agent & Tool Registry"`);
}
