...
async function renderModuleCard(
  ...
  const body = [
    ...
    "Предварительные свидетельства из code_refs — atlas-coder должен подтвердить конкретные символы.",
    "",
    "## Свидетельства из тестов",
    ...mod.testFiles.map((p) => `- Test file at ${p}:1`),
    "Предварительные свидетельства из test_refs — atlas-coder должен подтвердить покрытие тестами.",
    "",
    "## Связанные файлы",
    ...
    "",
    "## Связанные сценарии",
    "Не выявлены — atlas-extractor должен определить сценарии, использующие этот модуль.",
    "",
    "## Связанные решения",
    "Не выявлены — atlas-extractor должен определить решения, затрагивающие этот модуль.",
    "",
    "## Открытые вопросы",
