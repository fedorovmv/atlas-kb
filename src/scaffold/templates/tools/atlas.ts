...
// Wrapper script generated by `atlas init` at <repo>/.ai/atlas/bin/atlas.
// Works without package.json — only requires node in PATH.
const ATLAS_BIN = path.join(process.cwd(), ".ai", "atlas", "bin", "atlas");

async function runMemory(args: string[]) {
  try {
    const result = await execFileAsync(ATLAS_BIN, args, {
      cwd: process.cwd(),
